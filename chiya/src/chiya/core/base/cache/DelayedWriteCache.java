...
public class DelayedWriteCache<K, V> extends MapLockCache<K, V> {
	 ...
	 * @param task             异步写入的方法
	 */
	public DelayedWriteCache(ValueGetFunction<V, K> valueGetFunction, Task<V> task) {
	 ...
	 * @param valueGetFunction 获取对象的唯一标识
	 */
	public DelayedWriteCache(ValueGetFunction<V, K> valueGetFunction) {
		super(valueGetFunction);
	}
	...
	public void changeAll() {
		if (timeQueue == null) { return; }
		Loop.forEach(timeQueue.getKeys(), key -> task.task(get(key)));
		timeQueue.remove();
	}
