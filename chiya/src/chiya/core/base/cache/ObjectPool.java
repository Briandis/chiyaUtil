...
public class ObjectPool<K, V> {
	...
	/** 自定义获取实例方法 */
	private GetValueFunction<V> getValueFunction;
	/** 简易id生成的原子对象 */
	 ...
	 */
	private void timeQueueInit(Task<V> task) {
		this.task = task;
		timeQueue.start();
	}
	 ...
	 * @param task             对象如果被回收，则触发的方法
	 */
	public ObjectPool(GetValueFunction<V> getValueFunction, Task<V> task) {
	 ...
	 * @param task          对象如果被回收，则触发的方法
	 */
	public ObjectPool(Class<V> classTemplate, Task<V> task) {
	 ...
	 * @return 对象实例
	 */
	public V getPoolInstance(K key, GetValueFunction<V> getValueFunction) {
	 ...
	 */
	public void setGetValueFunction(GetValueFunction<V> getValueFunction) {
		this.getValueFunction = getValueFunction;
	}
