...
public class FileUtil {
	...
	public static List<String> getAllFile(String path) {
		...
		while (stack.size() != 0) {
			...
			if (nowFile.isDirectory()) { Loop.forEach(nowFile.list(), f -> stack.push(root + "\\" + f)); }
		}
		return listFile;
	}
	...
	public static List<String> getAllFile(String path, String... whiteList) {
		...
		while (stack.size() != 0) {
			...
			File nowFile = new File(root);
			if (nowFile.isFile()) {
				for (String whiteSuffix : whiteList) {
					if (StringUtil.eqString(StringUtil.getFileFormat(root), whiteSuffix)) { listFile.add(root); }
				}
			}
			if (nowFile.isDirectory()) { Loop.forEach(nowFile.list(), f -> stack.push(root + "\\" + f)); }
		}
