...
public class StringUtil {
	 ...
	 * @return 填充后的字符串
	 */
	public static String keepDecimal(int decimalCount, double number) {
		if (decimalCount < 1) { return String.valueOf((int) number); }
		String nowNumbe = String.valueOf(number);
		int index = findChar(nowNumbe, '.');
		int decimalSize = nowNumbe.length() - index;
		if (decimalSize > decimalCount) {
			return nowNumbe.substring(0, nowNumbe.length() - (decimalSize - decimalCount) + 1);
		} else {
			int maxLength = index + decimalCount + 1;
			char chars[] = new char[maxLength];
			for (int i = 0; i < maxLength; i++) {
				chars[i] = i < nowNumbe.length() ? nowNumbe.charAt(i) : '0';
			}
	 ...
	 * @return 填充后的字符串
	 */
	public static String keepDecimal(int decimalCount, String number) {
		return keepDecimal(decimalCount, NumberUtil.parseDouble(number));
	}
	...
	public static char[] zeroPadding(int len, int number, int base) {
		if (base < NumberUtil.BINARY || base > NumberUtil.HEX) { throw new IllegalArgumentException("base is less 2 or greater 16 ,base must is between 2 to 16"); }
	 ...
	 */
	public static int compareSize(String a, String b) {
	 ...
	 * @return 小于0是a &lt; b / 等于0是a=b / 大于0是a &gt; b
	 */
	public static int compareSizeNullIsMax(String a, String b) {
		if (a == null && b == null) { return 0; }
		if (a == null) { return 1; }
		if (b == null) { return -1; }
		int maxLength = a.length();
	 ...
	 * 格式化日志信息
	 * 
	 * @param paddingCount 填充的宽度
	 * @param paddingChar  填充字符
	 * @param data         填充的数据
	 * @return 格式化字符
	 */
	public static String formatPadding(int paddingCount[], String paddingChar, Object... data) {
		StringBuilder stringBuilder = new StringBuilder();
		if (data != null) {
			int index = 0;
			int length = 4;
			...
			for (Object obj : data) {
				...
				if (paddingCount != null && paddingCount.length > index) {
					// 根据数据长度添加剩余空格
					for (int i = 0; i < paddingCount[index] - length; i++) {
	...
	public static boolean inString(String key, String... value) {
		if (value != null) {
			for (String string : value) {
				if (eqString(key, string)) { return true; }
			}
		}
		return false;
	}
